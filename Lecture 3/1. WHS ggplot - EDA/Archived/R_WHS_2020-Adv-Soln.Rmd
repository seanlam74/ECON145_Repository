...
We also use the precalculated postion "lab_ypos" to position our label at the center of each coloured bar. 
```{r}
...
Exercise: Try plotting a line chart on another variable now.
```{r}
...
```{r}
data_agg <- data %>%
  group_by(Region) %>% summarise(Male = sum(Male), Female = sum(Female), Life_Expec = sum(Life_Expectancy_Both, na.rm= TRUE), 
